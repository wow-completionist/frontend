            ...
            output = (
              <div className="data-table">
                <table className="table table-hover table-dark">
                  ...
                  <tbody>
                    {this.props.transmogSetList.map((row) => {
                                let appearances = '';
                                if (row.visuals && this.props.visualMetaHash) {
                                    appearances = constants.MOG_SLOTS.map(slot => (
                                      <AppearanceText
                                        key={Math.random().toString(36).replace(/[^a-z]+/g, '')}
                                        categoryID={slot}
                                        visualIDs={row.visuals}
                                        visualMetaHash={this.props.visualMetaHash}
                                        fetchNameForItem={() => {}}
                                        removeVisualFromSet={() => {}}
...
const mapDispatchToProps = dispatch => ({ addTransmogSet: newSet => dispatch({ type: actionTypes.ADD_TRANSMOG_SET, data: newSet }), });
